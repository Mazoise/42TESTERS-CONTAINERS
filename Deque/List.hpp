...
/*   Updated: 2020/12/10 12:28:30 by hbaudet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	public:
			...
			explicit Deque(siz_type n, const value_type& val = value_type()): _n(0)
			{
				if (PRINT)
			...
			~Deque()
		{
			...
			//	MODIFIERS	//
			template <class InputIter>
  				void	assign(InputIter first, enable_if<InpuIter::input_iter, InputIter>::type last)
				{
					this->clear();
					this->insert(this->begin(), first, last);
				}
