...
run_tests ()
{
	...
	then
		...
		then
			echo -e "\e[31m\e[5m$WARNING""\t\e[0m\e[35m\e[1m""$M""_main.cpp seems to be invalid!\e[0m"
		else
			if [ "$(cat .error.log | wc -c)" != "0" ]
			then
				echo -e "\e[1m\e[31m\e[25m""$DEAD""\t""$M" "test does not compile!\e[0m"
			else
				...
				then
					valgrind ./"$BIN"/"$M"_ft > /dev/null 2> "$LEAK"/"$M"_leak
					if [ "$(grep "All heap blocks were freed -- no leaks are possible" "$LEAK"/"$M"_leak)" = "" ]
					then
						echo -e "\e[31m""$NOPE""\t""$M" "test leaks (shame!)"
					else
						if [ "$(grep "ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)" "$LEAK"/"$M"_leak)" = "" ]
						then
							echo -e "\e[31m""$NOPE""\t""$M" "Valgrind detected some errors"
						else
